...
public class AetherEventHandler {
    ...
    public void onEntityDropLoot(LivingDropsEvent event) {
        if (event.source instanceof EntityDamageSource) {
            ...
            if (source.getEntity() instanceof EntityPlayer) {
                ...
                if (currentItem != null && currentItem.getItem() instanceof ItemSkyrootSword
        ...
        if (event.entity instanceof EntityPlayer) {
            ...
            if (player.inventory.getCurrentItem() != null) {
                if (player.inventory.getCurrentItem()
                    .getItem() == ItemsAether.lightning_sword
                    || player.inventory.getCurrentItem()
                        .getItem() == ItemsAether.lightning_knife) {
                    event.setCanceled(true);
                }
            }
        }
    ...
    public void onEntityAttack(AttackEntityEvent event) {
        if (event.entityPlayer.getHeldItem() != null) {
            if (event.entityPlayer.getHeldItem()
                .getItem() == ItemsAether.flaming_sword) {
            ...
            } else if (event.entityPlayer.getHeldItem()
                .getItem() == ItemsAether.pig_slayer) {
                            ...
                            for (int j = 0; j < 20; j++) {
                                ...
                                event.target.worldObj.spawnParticle(
                                    ...
                                    (event.target.posX + (double) (itemRand.nextFloat() * event.target.width * 2.0F))
                                        ...
                                        - d * d3,
                                    (event.target.posY + (double) (itemRand.nextFloat() * event.target.height))
                                        - d1 * d3,
    ...
    public boolean isGravititeTool(Item stackID) {
        return stackID == ItemsAether.gravitite_shovel || stackID == ItemsAether.gravitite_axe
    ...
    public void onFall(LivingFallEvent event) {
        if (event.entityLiving instanceof EntityPlayer) {
            ...
            if (playerAether.getAccessoryInventory()
                ...
                || playerAether.getAccessoryInventory()
                    .isWearingGravititeSet()
                || playerAether.getAccessoryInventory()
